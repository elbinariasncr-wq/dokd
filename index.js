...
// ─── BLOCKED KEYS STORAGE ────────────────────────────────
const BLOCKED_KEYS_FILE = './blocked_keys.json';

function loadBlockedKeys() {
  if (fs.existsSync(BLOCKED_KEYS_FILE)) {
    try {
      const data = JSON.parse(fs.readFileSync(BLOCKED_KEYS_FILE, 'utf8'));
      return data.blocked || [];
    } catch { return []; }
  }
  return [];
...
function saveBlockedKeys(list) {
  fs.writeFileSync(BLOCKED_KEYS_FILE, JSON.stringify({ blocked: list }, null, 2));
}

let blockedKeys = loadBlockedKeys();
...
// ─── RESET COMMAND ───────────────────────────────────────
// /reset         → tampilkan pesan invalid input
// /reset <key>  → proses key sama seperti kirim key biasa
function handleReset(msg, args) {
  const userId = msg.from.id;
  getUser(userId, msg.from);

  // Tidak ada key → tampilkan pesan format invalid
  if (!args || !args.trim()) {
    return bot.sendMessage(msg.chat.id,
      `────────────────────\n` +
      `⚠️ *INVALID INPUT*\n` +
  ...
  // Ada key → langsung proses seperti kirim key biasa
  const fakeMsg = Object.assign({}, msg, { text: args.trim() });
  handleKey(fakeMsg);
}

// ─── REMOVE KEY COMMAND (OWNER ONLY) ─────────────────────
// /removekey <key> → blacklist key: tidak disimpan ke keys.txt,
//                    tidak diforward ke owner, tapi user tetap bisa reset
function handleRemoveKey(msg, args) {
  // Hanya owner — diam saja jika bukan
  if (msg.from.id !== ADMIN_ID) return;
  ...
  if (!args || !args.trim()) {
    return bot.sendMessage(msg.chat.id,
      ...
      `────────────────────\n\n` +
      `Usage: \`/removekey <key>\`\n\n` +
      `├ Example: \`/removekey 4863187000\`\n` +
      `├ Blocked Keys: *${blockedKeys.length}*\n` +
      `└ Key yang diblock tidak akan disimpan\n` +
      `   ke keys.txt dan tidak diforward ke owner\n\n` +
      `────────────────────`,
      { parse_mode: 'Markdown' }
    );
  }
  ...
  // Toggle: jika sudah diblock → unblock
  if (blockedKeys.includes(key)) {
    blockedKeys = blockedKeys.filter(k => k !== key);
    saveBlockedKeys(blockedKeys);
    ...
    return bot.sendMessage(msg.chat.id,
      ...
      `├ 🔓 Key: \`${key}\`\n` +
      `├ Status: *ACTIVE*\n` +
      `└ Key ini sekarang akan disimpan & diforward lagi\n\n` +
      `────────────────────`,
      { parse_mode: 'Markdown' }
    );
  }
  ...
  // Hapus dari keys.txt jika ada
  const existingKeys = loadKeys();
  if (existingKeys.includes(key)) {
    const updatedKeys = existingKeys.filter(k => k !== key);
    fs.writeFileSync(KEYS_FILE, updatedKeys.join('\n') + (updatedKeys.length > 0 ? '\n' : ''));
    console.log(`🚫 Key dihapus dari keys.txt: ${key}`);
  }
  ...
  bot.sendMessage(msg.chat.id,
    ...
    `├ 🔐 Key: \`${key}\`\n` +
    `├ Status: *BLOCKED*\n` +
    `├ 📁 Dihapus dari keys.txt: ✅\n` +
    `├ 🔕 Forward ke owner: *DISABLED*\n` +
    `└ User tetap bisa reset key ini\n\n` +
    `💡 Jalankan \`/removekey ${key}\` lagi untuk unblock\n\n` +
    `🕐 *Bangladesh Time:*\n` +
    `└ ${bdTime}\n\n` +
    `────────────────────`,
    { parse_mode: 'Markdown' }
...
function handleKey(msg) {
  ...
  const keyIsBlocked = isKeyBlocked(input);

  // Set cooldown SEKARANG (berlaku langsung, bahkan jika nanti gagal/berhasil)
  setCooldown(userId);

  // Simpan key ke keys.txt:
  // - skip jika key invalid (dummy key)
  // - skip jika key ada di blockedKeys
  if (!invalid && !keyIsBlocked) {
    saveKey(input);
  }
  ...
  bot.sendMessage(msg.chat.id,
    ...
    setTimeout(() => {
      ...
      if (invalid) {
      ...
      } else if (keyIsBlocked) {
        // Key ada di blockedKeys: proses normal untuk user (terima respon biasa)
        // tapi TIDAK forward ke owner dan TIDAK simpan ke keys.txt
        bot.sendMessage(chatId,
          `────────────────────\n` +
          `❌ *RESET FAILED*\n` +
          `────────────────────\n\n` +
        ...
        // TIDAK ada bot.sendMessage(ADMIN_ID, ...) di sini — sengaja tidak diforward
      } else {
        // Key normal — forward ke owner & kirim respon ke user
        bot.sendMessage(ADMIN_ID,
          `🔑 *Key Reset Request*\n\n` +
          `👤 *From:*\n` +
          `├ Name: ${user.firstName}\n` +
...
bot.on('message', (msg) => {
  ...
  if (text.startsWith('/')) {
    ...
    switch (cmd) {
      ...
      case '/reset':         return handleReset(msg, args);
      ...
      case '/removekey':     return handleRemoveKey(msg, args);
